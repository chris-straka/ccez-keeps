...
// checklist-item / attachment-name match (3); ties break by recency
// (newest updatedAt first). Never mutates inputs. The Kotlin port in
// android/synclogic Search.kt must keep the same tiers and order.
...
export function rankNotes(notes: Note[], query: string): Note[] {
  ...
  for (const note of notes) {
    ...
    else if (matchesExtras(note, q)) rank = 3;
...
function matchesExtras(note: Note, q: string): boolean {
